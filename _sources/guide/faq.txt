...
2. Disable operation validation by setting the ``command.disable_validation`` option to true on a command (this could have side effects)
3. Disable operation response processing by setting ``command.response_processing`` to 'raw' on a command (this will have side effects)
4. Disable operation model processing by setting ``command.response_processing`` to 'native' on a command (this will have side effects)
